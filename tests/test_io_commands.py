...
"""
    ...
    rows_dir = tmp_path / "rows"
    rows_dir.mkdir()
    (rows_dir / "customers.csv").write_text("customer_id\n1\n")
    (rows_dir / "customers.json").write_text(json.dumps([{"customer_id": 2}]))

    with pytest.raises(ValueError, match="duplicate entity artifact names"):
