...
def test_project_metadata_uses_public_name_and_stable_cli() -> None:
    ...
    assert "agent" not in metadata["keywords"]
    assert "mcp" not in metadata["keywords"]
...
def test_release_preflight_builds_exact_artifacts_without_publish_permissions() -> None:
    ...
    assert "uv run --no-sync scripts/check_release.sh" in workflow
    assert "SOURCE_DATE_EPOCH" in workflow
    assert "uv build --no-build-isolation" in workflow
    assert workflow.index("scripts/check_release.sh") < workflow.index(
