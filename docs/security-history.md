...
This page preserves the disposition and resolution history for four Low
findings originally accepted for the `1.0.0rc6` and `1.0.0` product scope. It
does not describe current accepted product-level issues.

The source is repository-wide Codex Security scan
`484dfa30-85d2-4059-b39b-2c52c9d0f5ed` of immutable commit
`2c714a6d4df75a1faab422055593fc50a2061a03`. All four findings are resolved in
stable `1.2.0`. Risk owner: [@wa-pis](https://github.com/wa-pis). The original
disposition was product-risk acceptance, not proof that broader deployment
models are safe. The independent RC6 review is recorded in the
...
Earlier review records remain available in the
[pre-RC review](security-review-2026-07-31.md) and
[RC2 review](security-review-2026-08-01-rc2.md).
...
a partial dataset, profile, or spec. Input byte, row, cell, and typed-model
limits remain independent defenses.
...
Generator and Trino MCP servers now replace typed FastMCP/Pydantic validation
details with the fixed `Tool arguments failed validation` error. Rejected values
and nested validation exceptions do not enter the MCP result.
...
response-free. Rejected values cannot enter SDK validation logs or retained
exception state.
