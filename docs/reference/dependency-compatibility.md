...
| `mcp-minimum` | Generator and Trino MCP transport schemas and real bounded stdio | `tests/test_mcp_generator_transport.py`, `tests/test_mcp_trino_transport.py`, `tests/test_mcp_sdk_roundtrip.py` |
...
MCP 2.2.0 was selected for compatibility testing on 2026-09-23. The shared
adapter supports both SDK server APIs while preserving wire aliases and the
per-request budget. SDK 2 unexpected tool exceptions produce the fixed
`Tool execution failed` message before SDK logging; validation errors remain
redacted. This does not add row-returning tools or relax transport limits.
