...
- [ ] Verify the official `gigachat` SDK license, transitive dependencies,
  release activity, structured-output API, and Python 3.11-3.14 compatibility.
- [ ] Select and document a bounded dependency range only if the SDK passes the
  base, `gigachat`, and `all` installation matrices.
- [ ] Define frozen typed settings, scope/authentication modes, run metadata,
  stable error categories, and an injected SDK protocol.
- [ ] Confirm the provider-neutral `AdvisorExchange` and artifact schemas need
  no breaking change; version any unavoidable serialized change.

## Provider Adapter

- [ ] Implement an optional `GigaChatAdvisorClient` without importing the SDK
  from the package root or deterministic core.
- [ ] Enforce official endpoints, mandatory TLS verification, optional CA
  bundle validation, and mutually exclusive environment-based auth modes.
- [ ] Map trusted instructions and untrusted exchange JSON into separate
  messages for one non-streaming strict-schema completion.
- [ ] Enforce request, response, output-token, timeout, retry, usage, and total
  invocation budgets.
- [ ] Reject abnormal finish reasons, multiple or missing choices, oversized
  content, malformed JSON, extra data, and schema-invalid proposals.
- [ ] Convert provider and SDK failures to fixed redacted errors and per-call
  bounded metadata without retaining raw requests or responses.
- [ ] Reuse the existing fingerprint, safety, review, and local field-label
  restoration boundaries without giving the adapter persistence authority.

## CLI And Packaging

- [ ] Add the optional `gigachat` extra and include it in `all` only after the
  dependency gates pass.
- [ ] Add `gigachat` to the existing `agent-advise --provider` choice while
  preserving all existing defaults and JSON contracts.
- [ ] Resolve credentials only from runtime environment or injected secret
  input; do not add credential-bearing CLI flags or config artifacts.
- [ ] Add a local-only `doctor --require-extra gigachat` capability smoke with
  fake transport, no credential resolution, and no network access.
- [ ] Keep missing-extra, missing-secret, invalid-scope, and TLS failures
  bounded, redacted, and free of tracebacks.

## Safety And Regression Tests

- [ ] Add fake-SDK unit tests for request roles, strict schema, settings,
  response parsing, finish reasons, retries, cancellation, and cleanup.
- [ ] Inspect the final outbound request for source-row, PII, credential,
  token, locally preserved value, and categorical-predicate sentinels.
- [ ] Prove invalid or provider-filtered output cannot change the workspace,
  persist a review, approve a spec, or generate records.
- [ ] Prove a valid proposal remains fingerprint-bound, review-gated, and uses
  the existing deterministic generator after approval.
- [ ] Add isolated-wheel tests for base, `gigachat`, and `all` profiles across
  Python 3.11-3.14 without a live provider.
- [ ] Add minimum/latest dependency, license, secret-scan, and import-boundary
