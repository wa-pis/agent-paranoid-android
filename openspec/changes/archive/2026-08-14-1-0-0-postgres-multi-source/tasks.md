...
- [x] Extend the typed field-scoped local category policy to source-qualified
...
- [x] Reuse the explicit `LocalCategoryField` scope for local files and
  canonical source-qualified PostgreSQL fields.
- [x] Require reviewed non-sensitive business enum/constant classification and
  fail-closed PII, secret, identifier, quasi-identifier, and free-text checks.
- [x] Enforce cardinality, value-length, and profiling budgets before retaining
  exact values or counts.
- [x] Preserve approved values, distributions, and conditional rules through
  local profile, specification, deterministic generation, and SQL export.
- [x] Replace all source literals at external provider boundaries and suppress
  them from default MCP responses, logs, errors, and source metadata.
- [x] Add focused tests for default-off behavior, exact approved values,
...
- [x] Enforce schema/table allowlists, read-only mode, identifier validation,
...
- [x] Add isolated unit tests with a fake driver; normal tests must not require
...
- [x] Update README, installation, configuration, product-validation, support,
...
- [x] Add isolated-wheel PostgreSQL/SQL smoke coverage without live database
  access.
- [x] Add an optional explicitly gated disposable-PostgreSQL execution check.
